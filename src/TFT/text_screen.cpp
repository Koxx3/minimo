...
/*
void DavegaTextScreen::reset(Adafruit_ILI9341* tft) {
    tft->fillRectangle(0, 0, tft->maxX() - 1, tft->maxY() - 1, COLOR_BLACK);
}

void DavegaTextScreen::update(Adafruit_ILI9341* tft, t_davega_data *data) {
    uint16_t session_data_color = progress_to_color(data->session_reset_progress, tft);
    uint16_t capacity_data_color = progress_to_color(data->mah_reset_progress, tft);
    float avg_speed_kph;
...
void DavegaTextScreen::heartbeat(Adafruit_ILI9341* tft, uint32_t duration_ms, bool successful_vesc_read) {
...
void DavegaTextScreen::_write_numeric_line(
        Adafruit_ILI9341* tft, float value, const char* units, const char* label, int lineno, uint16_t color) {
...
void DavegaTextScreen::_write_time_line(Adafruit_ILI9341* tft, uint32_t seconds, const char* label, int lineno, uint16_t color) {
    for (int i=0; i < MAX_LINE_LENGTH; i++)
        _line_buffer[i] = ' ';
...
void write_line_buffer(Adafruit_ILI9341 *tft, int x, int y, uint16_t color)
{
    ...
    //    tft->setFont(_big_font  ? &Terminal11x16 : &Terminal6x8);
