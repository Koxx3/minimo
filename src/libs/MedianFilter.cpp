...
int MedianFilter::getMeanWithoutExtremes(int extremeNb)
{

   int32_t val = 0;
   for (int i = extremeNb; i < medFilterWin - extremeNb; i++)
   {
      //Serial.println(data[sizeMap[i]]);
      val += data[sizeMap[i]];
   }
   return val / (medFilterWin - 2 * extremeNb);
}
